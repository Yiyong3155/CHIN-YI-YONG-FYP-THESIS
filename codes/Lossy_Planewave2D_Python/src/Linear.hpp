...
public:
  LinearSpectral2D(basix::FiniteElement<T> element,
        ...
        time_window = 0.0; time_bc_update = 0.0; time_scatter = 0.0;
        time_copy = 0.0; time_stiffness = 0.0; time_assemble = 0.0;
        time_scatter_rev = 0.0; time_solve = 0.0; time_f1_total = 0.0;

        c0 = speedOfSound; rho0 = density;
    ...
    void rk4(const T& startTime, const T& finalTime, const T& timeStep,
             ...
             const std::string& vtx_filename = "region_output") {
        ...
        auto V_sub = std::make_shared<fem::FunctionSpace<T>>(
            fem::create_functionspace(submesh_ptr, element_));
        ...
        if (output_interval > 0) {
            ...
            region_writer = std::make_unique<io::VTXWriter<T>>(
                ...
                std::vector<std::shared_ptr<const fem::Function<T>>>{u_sub}, "BP4");
        }

        if (mpi_rank == 0) {
            std::cout << "\n📊 Submesh VTX output setup (2D):\n";
        ...
        if (region_writer) {
            ...
            u_sub->interpolate(*u_n, sub_cell_map); region_writer->write(startTime);
        }
        ...
        while (t < tf) {
            ...
            if (region_writer && step % output_interval == 0) {
                kernels::copy(*u_, *u_n->x()); u_n->x()->scatter_fwd();
                u_sub->interpolate(*u_n, sub_cell_map); region_writer->write(t);
            }
        ...
        if (mpi_rank == 0) {
            ...
            if (output_interval > 0)
                std::cout << "   VTX submesh written to vtx_output/" << vtx_filename << ".bp\n";
            std::cout << "   Region: [" << region_min[0] << ", " << region_min[1]
                      << "] to [" << region_max[0] << ", " << region_max[1] << "]\n";
            std::cout << "   Submesh cells: " << submesh_cells.size() << "\n";
            std::cout << "========================================\n";
...
private:
    ...
    basix::FiniteElement<T> element_;
    ...
    std::span<T> c_;
    double time_window, time_bc_update, time_scatter, time_copy;
    double time_stiffness, time_assemble, time_scatter_rev, time_solve, time_f1_total;
    std::span<T> u_n_array_cache, v_n_array_cache, g_array_cache;
};
...
public:
    LinearSpectral3D(
        ...
        std::shared_ptr<fem::Function<T>> speedOfSound,
        std::shared_ptr<fem::Function<T>> density,
        const T& sourceFrequency, const T& sourceAmplitude, const T& sourceSpeed,
        ...
        element_ = element;
        region_min = eval_region_min; region_max = eval_region_max;
        ...
        c0 = speedOfSound; rho0 = density;
        freq = sourceFrequency; w0 = 2 * M_PI * sourceFrequency;
        p0 = sourceAmplitude; s0 = sourceSpeed;
        period = 1.0 / sourceFrequency; window_length = 4.0;
        mesh = Mesh; ft = FacetTags;
    ...
    void rk4(const T& startTime, const T& finalTime, const T& timeStep,
        ...
        if (mpi_rank == 0) {
            double sum = time_window + time_bc_update + time_scatter + time_copy
                       + time_stiffness + time_assemble + time_scatter_rev + time_solve;
            std::cout << "\n========================================\n";
            std::cout << "✅ Simulation complete!\n";
            if (output_interval > 0)
                std::cout << "   VTX submesh written to vtx_output/" << vtx_filename << ".bp\n";
            ...
            std::cout << "   Submesh cells: " << submesh_cells.size() << "\n";
            std::cout << "\n========== TIMING BREAKDOWN ==========\n";
            std::cout << "f1() total:      " << time_f1_total*1000 << " ms\n";
            std::cout << "  Window:        " << time_window*1000 << " ms\n";
            std::cout << "  BC update:     " << time_bc_update*1000 << " ms\n";
            std::cout << "  Scatter fwd:   " << time_scatter*1000 << " ms\n";
            std::cout << "  Copy:          " << time_copy*1000 << " ms\n";
            std::cout << "  Stiffness:     " << time_stiffness*1000 << " ms\n";
            std::cout << "  Assembly:      " << time_assemble*1000 << " ms\n";
            std::cout << "  Scatter rev:   " << time_scatter_rev*1000 << " ms\n";
            std::cout << "  Solve:         " << time_solve*1000 << " ms\n";
            std::cout << "  Overhead:      " << (time_f1_total-sum)*1000 << " ms\n";
            std::cout << "======================================\n";
        }
    }
