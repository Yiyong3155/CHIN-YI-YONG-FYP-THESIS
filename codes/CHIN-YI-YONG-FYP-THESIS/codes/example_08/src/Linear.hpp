...
public:
 ...
 void rk4(const T& startTime, const T& finalTime, const T& timeStep) {
    // ========================================================================
    // SETUP: Create evaluation grid in custom region ONCE before simulation
    // ========================================================================
    
    const std::size_t Nx = grid_nx;
    const std::size_t Ny = grid_ny;
    
    // Create evaluation points in custom region
    std::vector<double> point_coordinates(3 * Nx * Ny);
    for (std::size_t i = 0; i < Nx; ++i) {
        for (std::size_t j = 0; j < Ny; ++j) {
            std::size_t idx = i * Ny + j;
            point_coordinates[3*idx] = 
                region_min[0] + i * (region_max[0] - region_min[0]) / (Nx - 1);
            point_coordinates[3*idx + 1] = 
                region_min[1] + j * (region_max[1] - region_min[1]) / (Ny - 1);
            point_coordinates[3*idx + 2] = 0.0;  // z = 0 for 2D
        }
    }
    
    // Find which cells contain these points (ONCE, before simulation)
    ...
    // Collect points that are inside the mesh on this process
    ...
    if (mpi_rank == 0) {
        std::cout << "\n📊 Custom region evaluation setup:\n";
        std::cout << "   Grid: " << grid_nx << " x " << grid_ny << " = " 
                  << (grid_nx * grid_ny) << " points\n";
        std::cout << "   Points on rank 0: " << num_points_local << "\n";
    ...
    T tn;
    int data_counter = 0;
    const int numStepPerPeriod = static_cast<int>(period / dt);
    ...
    while (t < tf) {
        ...
        // ====================================================================
        // ✅ EVALUATE REAL PRESSURE ON CUSTOM REGION ONLY
        // ====================================================================
        
        if (step % numStepPerPeriod == 0) {
            // Copy current solution for evaluation
            kernels::copy(*u_, *u_n->x());
            u_n->x()->scatter_fwd();
            
            // Evaluate pressure at grid points in custom region
            u_n->eval(points_on_proc, {num_points_local, 3}, cells, u_eval,
                      {num_points_local, 1});
            u_value = u_eval.data();
            
            // Write to file (each MPI process writes its portion sequentially)
            MPI_Barrier(MPI_COMM_WORLD);
            
            for (int i = 0; i < mpi_size; ++i) {
                if (mpi_rank == i) {
                    std::string fname = "data/pressure_field_" + 
                                        std::to_string(data_counter) + ".txt";
                    
                    std::ofstream txt_file(fname, std::ios_base::app);
                    
                    // First process writes header
                    if (i == 0) {
                        // Use scientific notation for header (readable)
                        txt_file << std::scientific << std::setprecision(10);
                        txt_file << "# Time: " << t << "\n";
                        txt_file << "# Grid: " << grid_nx << " x " << grid_ny << "\n";
                        txt_file << "# Region: [" << region_min[0] << ", " << region_min[1] 
                                 << "] to [" << region_max[0] << ", " << region_max[1] << "]\n";
                        txt_file << "x,y,pressure\n";
                    }
                    
                    // Switch to FIXED notation for data (ParaView compatible!)
                    txt_file << std::fixed << std::setprecision(6);
                    
                    // Write REAL evaluated pressure data
                    for (std::size_t j = 0; j < num_points_local; ++j) {
                        txt_file << *(p_value + 3 * j) << ","
                                 << *(p_value + 3 * j + 1) << ","
                                 << *(u_value + j) << "\n";
                    }
                    txt_file.close();
            ...
            if (mpi_rank == 0) {
                std::cout << "✅ Written: data/pressure_field_" << data_counter 
                          << ".txt (REAL pressure, custom region only, fixed format)\n";
            }
    ...
    if (mpi_rank == 0) {
        ...
        std::cout << "   Region data files: " << data_counter << " files\n";
        std::cout << "   Region: [" << region_min[0] << ", " << region_min[1] 
                  << "] to [" << region_max[0] << ", " << region_max[1] << "]\n";
        std::cout << "   Grid: " << grid_nx << " x " << grid_ny << " = " 
                  << (grid_nx * grid_ny) << " points\n";
        std::cout << "   Points evaluated per file: ~" << num_points_local << " (on rank 0)\n";
        std::cout << "   ✅ REAL pressure, ParaView-compatible fixed format\n";
        std::cout << "========================================\n";
    }
}
...
private:
    ...
    std::span<T> g_array_cache;
     ...
     std::array<T, 2> region_min;
     std::array<T, 2> region_max;
     std::size_t grid_nx;
     std::size_t grid_ny;
};
...
public:
   LinearSpectral3D(
        ...
        // ✅ ADDED: Initialize timing variables
        time_window = 0.0;
        time_bc_update = 0.0;
        time_scatter = 0.0;
        time_copy = 0.0;
    ...
    void f1(T& t,
        ...
        // ✅ MODIFIED: Update boundary condition using cached array
        std::fill(g_array_cache.begin(), g_array_cache.end(), 
                  window * p0 * w0 / s0 * cos(w0 * t));
        auto t2 = std::chrono::high_resolution_clock::now();
        time_bc_update += std::chrono::duration<double>(t2 - t1).count();
        ...
        // ✅ MODIFIED: Copy using cached array
        std::span<const T> u_span = u->array();
        std::copy(u_span.begin(), u_span.end(), u_n_array_cache.begin());
        auto t4 = std::chrono::high_resolution_clock::now();
        time_copy += std::chrono::duration<double>(t4 - t3).count();
    ...
    std::string fname;
     ...
     // ✅ USE USER-DEFINED 3D GRID PARAMETERS
    const std::size_t Nx = grid_nx;
    const std::size_t Ny = grid_ny;
    const std::size_t Nz = grid_nz;  // ✅ ADDED
    
    // ✅ CREATE EVALUATION POINTS USING USER-DEFINED REGION
    std::vector<double> point_coordinates(3 * Nx * Ny * Nz);  // ✅ 3D grid
    for (std::size_t i = 0; i < Nx; ++i) {
        for (std::size_t j = 0; j < Ny; ++j) {
            for (std::size_t k = 0; k < Nz; ++k) {  // ✅ ADDED z-loop
                std::size_t idx = i * Ny * Nz + j * Nz + k;
                point_coordinates[3*idx] = 
                    region_min[0] + i * (region_max[0] - region_min[0]) / (Nx - 1);
                point_coordinates[3*idx + 1] = 
                    region_min[1] + j * (region_max[1] - region_min[1]) / (Ny - 1);
                point_coordinates[3*idx + 2] = 
                    region_min[2] + k * (region_max[2] - region_min[2]) / (Nz - 1);  // ✅ z-coordinate
            }
        }
    }
    
    // ✅ COMPUTE COLLISION DETECTION FOR EVALUATION POINTS
    const int tdim = mesh->topology()->dim();
    mesh->topology()->create_entities(tdim);
    auto map = mesh->topology()->index_map(tdim);
    const std::int32_t num_entities = map->size_local() + map->num_ghosts();
    ...
    for (std::size_t i = 0; i < Nx*Ny*Nz; ++i) {  // ✅ CHANGED to Nx*Ny*Nz
        auto link = colliding_cells.links(i);
        if (link.size() > 0) {
            points_on_proc.push_back(point_coordinates[3*i]);
            points_on_proc.push_back(point_coordinates[3*i + 1]);
    ...
    while (t < tf) {
        ...
        // ----------------------------------------------------------------------
        // ✅ COLLECT DATA IN USER-DEFINED REGION
...
private:
    ...
    std::array<T, 3> region_min;
    std::array<T, 3> region_max;
    std::size_t grid_nx;
    std::size_t grid_ny;
    std::size_t grid_nz;
    int mpi_rank, mpi_size;
